...
`SpotClean` provides functions `read10xRaw()` and `read10xSlide()` to directly read these 10x Space Ranger output files and generate a gene-by-spot count matrix and slide metadata in R. Here for the purpose of demonstration, we have two built-in example objects `mbrain_raw` and `mbrain_slide_info`. `mbrain_raw` is a subset of the gene-by-spot count matrix from `read10xRaw()`, containing the top 100 highest expressed genes. `mbrain_slide_info` is the direct output of `read10xSlide()`.
