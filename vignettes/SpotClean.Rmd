...
We combine count matrix and slide information together as one single slide object with class `SummarizedExperiment`:
...
Function `VisualizeLabel()` shows the spot labels. You can specify the column name of labels in the slide information, or manually provide a vector of characteristic labels corresponding to each spot. For example, we can plot their tissue/background labels, which has been stored in the input slide information:
...
Function `VisualizeHeatmap()` draws a heatmap of values at every spot in the 2-D slide space. For example, we can plot the total UMI counts in every spot, which again has been stored in the input slide information:

```{r,fig.width=5, fig.height=4}
VisualizeHeatmap(slide_obj,"total_counts")
```

Similar as `VisualizeLabel()`, You can also provide a certain gene name in raw count matrix to plot, or manually give a vector of values corresponding to each spot. For example, the expression of Mbp gene can be visualized:
...
We can visualize the gene expressions after 10 iterations of decontamination:
...
The metadata now contains more information including parameter estimations in the SpotClean model and measurements of contamination level. One important measurement people are interested in is the amount of contaminated expressions in observed expressions, i.e. expressions not originated from a tissue spot, but from other tissue spots due to spot swapping. Our model is able to estimate the proportion of contaminated expressions in each tissue spot:

```{r}
summary(decont_obj@metadata$contamination_rate)
```

This indicates around 30% expressions in observed data came from spot swapping contamination on average.
...
`Seurat` is one of the most widely-used package for downstream analyses of single-cell and spatial data. We provide function `ConvertToSeurat()` to convert our slide object to Seurat spatial object. Note that Seurat requires full input of the spatial folder. In the example above it will be the `spatial` folder.
