...
Given the fact that spot swapping happens ubiquitously for all genes across the whole slide, SpotClean has the potential to improve the accuracy of any marker gene-based inferences including tissue type annotation, cell type decomposition, integration with single-cell RNA-seq data, and their downstream analyses. SpotClean also improves the identification of spatially variable genes since it enhances the signals of high-expressed regions. 

SpotClean improves the identification of differentially expressed genes between two groups of spots by enhancing the fold change. The p-value, however, sometimes is not improved due to the increased variability within the spot groups.

For clustering analyses, SpotClean didn't show significant improvement since the cluster signals are already strong enough in raw data.

# Detailed Steps

## Load count matrix and slide information from 10x Space Ranger output

Currently, the most widely-used spatial transcriptomics protocol is 10x Visium. `SpotClean` provides functions to directly read 10x Space Ranger output files and generate a feature-by-barcode count matrix and slide information in R. Public 10x datasets can be found [here](https://support.10xgenomics.com/spatial-gene-expression/datasets/).

`SpotClean` contains a small subset of 10x data corresponding to the top 100 highest expressed genes of [V1_Adult_Mouse_Brain](https://support.10xgenomics.com/spatial-gene-expression/datasets/1.0.0/V1_Adult_Mouse_Brain). Below we show how to process raw 10x Space Ranger output files to get this same example subset of data.
